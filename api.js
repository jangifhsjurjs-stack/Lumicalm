...
class TextSentimentService {
  ...
  runTextAnalysis(userInput) {
    ...
    if (input.includes("เครียด") || input.includes("กังวล") || input.includes("ตื่นเต้น") || input.includes("กดดัน") || input.includes("วิตก") || input.includes("ภาระ") ||
        input.includes("หมดไฟ") || input.includes("คิดมาก") || input.includes("ใจสั่น") || input.includes("แน่นหน้าอก") || input.includes("ไม่ไหว") || input.includes("เซ็ง") ||
    ...
    if (input.includes("นอนไม่หลับ") || input.includes("เหนื่อย") || input.includes("เพลีย") || input.includes("ล้า") || input.includes("อยากนอน") || input.includes("ฝันร้าย") ||
        input.includes("หมดไฟ") || input.includes("ไม่ไหว") ||
    ...
    // 7. Happy / Calm / Contentment (ความสุข ความสงบ และความผ่อนคลาย)
    if (input.includes("มีความสุข") || (input.includes("สดชื่น") && !input.includes("ไม่สดชื่น")) || input.includes("ผ่อนคลาย") || input.includes("สบายใจ") || input.includes("อารมณ์ดี") ||
        input.includes("happy") || input.includes("joy") || input.includes("content") || input.includes("peaceful") || input.includes("calm")) {
      return {
        colorHex: "#2dd4bf", // 525nm / 460nm Refreshing Wave
        colorReason: "คลื่นแสง 525 นาโนเมตร ผสาน 460 นาโนเมตร (Refreshing Wave) ช่วยสร้างบรรยากาศโปร่งสบาย สดชื่น และส่งเสริมสภาวะความสุขสงบทางอารมณ์อย่างเป็นธรรมชาติ",
        techniqueName: "โปรแกรมคลื่นสดชื่นผ่อนคลาย (Refreshing Wave 4-2-4-2)",
        tagline: "จังหวะฝึกเพื่อคืนความสดชื่น เบิกบานใจ และบ่มเพาะสภาวะความสุขสงบภายใน",
        phases: [
          { label: "หายใจเข้าสดชื่น", duration: 4, type: "in" },
          { label: "กลั้นรับรู้ความสุข", duration: 2, type: "hold" },
          { label: "หายใจออกผ่อนคลาย", duration: 4, type: "out" },
          { label: "กลั้นประคองใจสงบ", duration: 2, type: "hold" }
        ],
        affirmation: "รับรู้และขอบคุณความสุขสงบในปัจจุบัน ร่างกายและจิตใจเปี่ยมด้วยพลังบวกและความเบาสบาย",
        emotionProfile: {
          category: "happy",
          primary: { label: "มีความสุขสงบ", labelEn: "Happy & Content", intensity: 8 },
          secondary: [
            { label: "สดชื่นผ่อนคลาย", labelEn: "Refreshed & Calm", intensity: 8 },
            { label: "สบายใจ", labelEn: "Peaceful", intensity: 7 }
          ],
          reflection: "คุณกำลังอยู่ในสภาวะอารมณ์ที่เบิกบาน สงบ และผ่อนคลาย เป็นช่วงเวลาที่ยอดเยี่ยมในการรับรู้คุณค่าและขอบคุณพลังบวกในชีวิต"
        }
      };
    }

    // 8. Neutral / Numb / Exploring (สภาวะเป็นกลาง ว่างเปล่า หรือกำลังสำรวจตนเอง)
    if (input.includes("อธิบายไม่ถูก") || input.includes("เฉยๆ") || input.includes("ว่างเปล่า") || input.includes("ไม่แน่ใจ") || input.includes("เบื่อ") ||
        input.includes("numb") || input.includes("blank") || input.includes("empty") || input.includes("bored")) {
      return {
        colorHex: "#5c6bc0", // 440nm Grounding Wave
        colorReason: "คลื่นแสงอ่อนโยนช่วยสร้างสมดุล ประคองสติให้อยู่กับปัจจุบัน และเปิดพื้นที่ปลอดภัยสำหรับการสังเกตและสำรวจความรู้สึกตนเอง",
        techniqueName: "ฝึกปรับสมดุลสติสู่ปัจจุบัน (Gentle Grounding 4-4-4-4)",
        tagline: "จังหวะฝึกสติสี่ขั้นตอนอย่างอ่อนโยน ช่วยรวมจุดสนใจและเชื่อมโยงร่างกายกลับสู่ปัจจุบันขณะ",
        phases: [
          { label: "หายใจเข้าอย่างอ่อนโยน", duration: 4, type: "in" },
          { label: "กลั้นประคองสติ", duration: 4, type: "hold" },
          { label: "หายใจออกปล่อยวาง", duration: 4, type: "out" },
          { label: "กลั้นรับรู้สภาวะ", duration: 4, type: "hold" }
        ],
        affirmation: "ไม่จำเป็นต้องเร่งรีบหรือรู้สึกอะไรเป็นพิเศษในตอนนี้ เพียงแค่รับรู้ลมหายใจและอนุญาตให้ตัวเองได้อยู่กับปัจจุบันอย่างมั่นคง",
        emotionProfile: {
          category: "neutral",
          primary: { label: "สภาวะเป็นกลาง/ว่างเปล่า", labelEn: "Neutral & Grounding", intensity: 5 },
          secondary: [
            { label: "เฉยๆ", labelEn: "Neutral", intensity: 5 },
            { label: "สำรวจตนเอง", labelEn: "Exploring", intensity: 5 }
          ],
          reflection: "สภาวะที่คุณรู้สึกว่างเปล่าหรือเฉยๆ เป็นโอกาสให้คุณได้หยุดพัก สังเกตสิ่งที่เกิดขึ้นในร่างกาย และค่อยๆ ปรับสมดุลกลับสู่ปัจจุบันขณะอย่างเป็นธรรมชาติ"
        }
      };
    }

    // 9. Default Fallback: Forest Green Calm (สภาวะผ่อนคลายทั่วไป)
    return {
      colorHex: "#66bb6a", // 525nm Green
