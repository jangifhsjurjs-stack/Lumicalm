...
class LunicalmApp {
  ...
  init() {
    ...
    // 8. Global user-gesture autoplay unlock
    const unlockAudio = () => {
      if (this.audioSynthesizer) {
        this.audioSynthesizer.ensureContext();
      }
    };
    window.addEventListener('click', unlockAudio, { once: true });
    window.addEventListener('touchstart', unlockAudio, { once: true });

    // 9. Initialize API status indicator
  ...
  updateApiStatusIndicator(status, customLabel) {
    const dot = document.getElementById('api-dot');
    const label = document.getElementById('api-status-label');
    
    if (dot) {
      dot.className = 'status-dot';
      if (status === 'analyzing' || status === 'simulated') {
    ...
    if (label) {
      ...
      } else if (status === 'analyzing' || status === 'simulated') {
        label.innerText = 'กำลังวิเคราะห์สภาวะอารมณ์...';
      } else if (status === 'offline' || status === 'error') {
        label.innerText = 'ระบบการวิเคราะห์ขัดข้องชั่วคราว (Offline)';
      } else {
  ...
  applyColorTherapy(color) {
    ...
    // Scoped active state management for Chromotherapy presets
    document
      .querySelectorAll('#chromotherapy-presets .premium-preset-card')
      .forEach(card => {
  ...
  applyBreathingTechnique(tech) {
    ...
    // Scoped active state management for Breathing presets
    document
      .querySelectorAll('#breathing-presets .premium-preset-card')
      .forEach(card => {
        card.classList.toggle(
          'active',
  ...
  showToast(message, actionLabel, onAction) {
    let toast = document.getElementById('lumicalm-toast');
    if (!toast) {
      toast = document.createElement('div');
      toast.id = 'lumicalm-toast';
      toast.className = 'lumicalm-toast';
    ...
    msgSpan.className = 'lumicalm-toast-message';
    ...
    if (actionLabel && onAction) {
      ...
      actionBtn.className = 'lumicalm-toast-action';
  ...
  renderPresets() {
    ...
    DEFAULT_BREATHING_PRESETS.forEach(tech => {
      ...
      card.addEventListener('click', () => {
        ...
        this.showToast('ใช้งานจังหวะนี้แล้ว', 'ไปยังหน้าฝึก (Canvas)', () => {
  ...
  async toggleAudioEngine(isChecked) {
    if (isChecked) {
      await this.audioSynthesizer.start();
      if (this.isPlaying) {
  ...
  startBreathing() {
    ...
    // Ensure audio context is resumed upon user interaction (clicking Play)
    if (this.audioSynthesizer) {
      this.audioSynthesizer.ensureContext();
    }
    const audioSwitch = document.getElementById('audio-switch');
    if (audioSwitch && audioSwitch.checked && !this.audioSynthesizer.isActive) {
      this.audioSynthesizer.start();
    }

    document.getElementById('play-btn-text').dataset.langKey = 'btn-pause';
    document.getElementById('play-btn-text').innerText = (window.LUNICALM_STRINGS?.['btn-pause']?.[window.LunicalmLang?.current || 'th']) || 'หยุดชั่วคราว';
  ...
  stopBreathing() {
    ...
    document.getElementById('play-btn-text').dataset.langKey = 'btn-play';
    document.getElementById('play-btn-text').innerText = (window.LUNICALM_STRINGS?.['btn-play']?.[window.LunicalmLang?.current || 'th']) || 'ฝึกการหายใจ';
    ...
    if (document.getElementById('audio-switch').checked && this.audioSynthesizer) {
      this.audioSynthesizer.fadeForBreathingPause(0.8);
    }
  ...
  async runAiTherapist() {
    ...
    this.updateApiStatusIndicator('analyzing', 'กำลังวิเคราะห์สภาวะอารมณ์...');
    ...
    try {
      ...
      this.updateApiStatusIndicator('online', 'ระบบวิเคราะห์สภาวะอารมณ์ด้วยข้อความ (Text-Based Sentiment Analyzer)');
      ...
      btn.disabled = false;
      btn.innerText = 'วิเคราะห์และปรับแต่งคลื่นผ่อนคลาย';
      this.updateApiStatusIndicator('offline', 'เกิดข้อผิดพลาดในการวิเคราะห์');
      this.logConsole(`Sentiment analysis error: ${err.message}`, "error");
    }
  }
